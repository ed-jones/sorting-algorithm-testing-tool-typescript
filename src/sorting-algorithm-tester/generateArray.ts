...
export default function generateArray<T>(testType: TestType, length: number, generateValue: IGenerate<T>){
    let list: T[] = [];
    switch(testType) {
        ...
        case TestType.SHUFFLED:
            const presorted = generateArray<T>(TestType.PRESORTED, length, generateValue);
