...
export enum Efficiency {
    N_LOG_SQUARED = "n log^2(n)", N_SQUARED = "n^2", N_LOG_N = "n log(n)", LOG_N = "log(n)", N = "n", UNKNOWN = "Unknown",
}
...
export enum TestType {
    PRESORTED = "Presorted", RANDOM = "Random", REVERSE_SORTED = "Reverse Sorted", SHUFFLED = "Shuffled", EMPTY = "Empty",
}
...
export default class SortingAlgorithmTester<T> {
    ...
    private tests: {testType: TestType, count: number, efficiency?: Efficiency}[] = [];

    constructor(sortingAlgorithm: ISortingAlgorithm<T> , compare: ICompare<T>, generate: IGenerate<T>) {
    ...
    runTests() {
        this.tests.forEach((test) => {
            ...
            const unsortedList = this.generateList(test.testType, test.count);
            let comparisonCount = 0;

            const sortedList = this.sortingAlgorithm(unsortedList, (arg0: T, arg1: T) => {
                comparisonCount++;
            ...
            const closestEfficiency = this.getClosestEfficiency(comparisonCount, sortedList.length);
    ...
    private getClosestEfficiency(comparisonCount: number, listLength: number): Efficiency {
    ...
    private generateList(testType: TestType, length: number): T[] {
        let list: T[] = [];
        switch(testType) {
            case TestType.PRESORTED:
                for (let i = 0; i < length; i++) {
                    list.push(this.generate(i));
                }
            break;
            ...
            case TestType.SHUFFLED:
                const presorted = this.generateList(TestType.PRESORTED, length);
                list = presorted.sort(() => Math.random() - 0.5);
            break;
            case TestType.RANDOM:
                for (let i = 0; i < length; i++) {
                    list.push(this.generate(Math.floor(Math.random()*length)));
                }
            break;
            case TestType.EMPTY:
            break;
