...
export default class SortingAlgorithms<T> {
  ...
  heapSort: ISortingAlgorithm<T> = (unsorted, compare) => {
    ...
    function siftDown(unsorted: T[], startPos: number, endPos: number) {
      ...
      while (rootPos * 2 + 1 <= endPos) {
        ...
        if (
          childPos + 1 <= endPos &&
          compare(unsorted[childPos], unsorted[childPos + 1]) === -1
        ) {
          childPos++;
        }
        if (compare(unsorted[rootPos], unsorted[childPos]) === -1) {
          [unsorted[rootPos], unsorted[childPos]] = [
            unsorted[childPos],
            unsorted[rootPos],
          ];
          rootPos = childPos;
  ...
  quickSort: ISortingAlgorithm<T> = (unsorted, compare) => {
    if (unsorted.length) {
      ...
      return ([] as T[]).concat.apply([] as T[], [
        this.quickSort(less, compare),
        h,
        this.quickSort(more, compare),
      ]);
    } else return [];
  ...
  selectionSort: ISortingAlgorithm<T> = (unsorted, compare) => {
    ...
    for (let i = 0; i < len; i++) {
      ...
      for (let j = i + 1; j < len; j++) {
        if (compare(unsorted[j], unsorted[minAt]) === -1) minAt = j;
      }
